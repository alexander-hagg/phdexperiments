...
for iii=1:length(shownRuns)
    ...
    [stats{run},p,d,collect] = getModelComparisonStats(data, p, d, experimentNames);%, 'includesamples', false);
    
    for i=1:length(dimReduxMethods)
        disp('===============================================');
        disp(['Get optimal regions with ' dimReduxMethods{i} '/DBSCAN']);
        disp('===============================================');
        runData{run,i}.method = dimReduxMethods{i};
        %if isnan(numDims) || ~strcmp(dimReduxMethods{i},'tSNE')
            runData{run,i}.numDims = ceil(intrinsic_dim(stats{run}.agg.optimaLocations, 'MLE'));
        %else
        %    runData{run,i}.numDims = 2;
        %end
        
        [runData{run,i}.estimatedLabels, runData{run,i}.reducedParSpace, ~, runData{run,i}.epsilon, runData{run,i}.coreneighbours, runData{run,i}.clusters] = ...
            dimReducedClustering( stats{run}.agg.optimaLocations, dimReduxMethods{i}, runData{run,i}.numDims);
        disp([int2str(max(runData{run,i}.estimatedLabels))  ' clusters found, ' int2str(sum(runData{run,i}.estimatedLabels==0)) '/' int2str(size(runData{run,i}.reducedParSpace,1))  ' samples unassigned']);
        
        disp(['Analyze optimal regions']);
        [runData{run,i}.clusters.analysis] = getClusterAnalysis(stats{run},p,d,collect, data, runData{run,i}.clusters, runData{run,i}.estimatedLabels, runData{run,i}.reducedParSpace);
...
for iii=1:length(shownRuns)
    run = shownRuns(iii);
    sOrg = stats{run}.agg.optimaLocations;
    for i=1:length(dimReduxMethods)
        sRed = runData{run,i}.reducedParSpace;
        ...
        %eks(run,i) = norm((Dd-Dq),'fro').^2;
        
        %% Quality of k-neighbourhood ranking
        %enx(run,i) = m_enx(Dd,Dq)
        
        %% Silhouette Coefficient
        %silh(run,i) = m_silhouette(stats{run}.agg.optimaLocations,runData{run,i}.estimatedLabels);
        
        %% Calinski-Harabasz index
        %calhar(run,i) = m_calinski(stats{run}.agg.optimaLocations,runData{run,i}.estimatedLabels);
        
        %% G+ index
        Gplus(run,i) = m_gplus(pdist2(sRed,sRed), runData{run,i}.estimatedLabels)
        
        %% Number of clusters
        numClusters(run,i) = length(unique(runData{run,i}.estimatedLabels));
    end
end
