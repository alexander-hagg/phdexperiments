...
int main(int argc, char* argv[])
{
    ...
    using namespace nn;
    Elman<Neuron<PfWSum<>, AfTanhNoBias<> >, Connection<> > nn(numInputs, numHiddenNeurons, 2);
    ...
    // std::cout << trajFilename << std::endl;
    remove( trajFilename.c_str() );
   
    std::ofstream traj(trajFilename.c_str());
    
    if (debug) std::cout << "Simulating" << std::endl;
