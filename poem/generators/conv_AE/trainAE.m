function model = trainAE(model,data,numEpochs,maxBatchSize,learnRate)
...
trainData = readall(data.trainStore);
latent = getAELatent(model,trainData{:,1});
[~,model.normalization] = mapminmax(latent,0,1);
