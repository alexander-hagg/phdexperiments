...
%------------- BEGIN CODE --------------  
    ...
    %    mu = parents - p.concept.value.mu; % Move mutation distribution to centroid
    %    sigma = p.concept.value.Sigma;
    %    R = chol(sigma);
    %    mutation = mu + randn(p.nChildren,size(parents,2))*R*p.mutSigma;
    %else
