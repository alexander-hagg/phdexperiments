function [cD, cL] = xfoilCdCl(coord,tmpdir, AOA,RE,MACH,varargin)
...
% Jul 2016; Last revision: 14-Aug-2017

%------------- BEGIN CODE --------------

%% Move to tmp folder and set file names
wd = tmpdir; if ~exist(wd,'dir');system(['mkdir ' wd]); end; cd(wd);

[status, result] = system(['cd ' wd]);
if (status~=0), disp(result); disp(['no directory in: ' wd]); end

ID = [int2str(randi(100000))];[status, result] =system(['timeout 5s xfoil < xfoil' ID '.inp > xfoil' ID '.out']);
...
%% Run XFoil
% XFoil can hang with some solutions, so kill it if it does and give a NaN
[status, ~] = system(['timeout 5s xfoil < xfoil' ID '.inp > xfoil' ID '.out']);
if (status~=0)
