...
%% Acquisition Loop
trainingTime = []; trainingPredictionTime = []; illumTime = []; peTime = [];h = [];
while nSamples <= p.nTotalSamples
    ...
    for iModel = 1:size(value,2)
        ...
        d.params{iModel}.functionEvals = 100;
        model{iModel} = feval(['train' d.params{iModel}.name], observation, value(:,iModel), d.params{iModel});
        %if ~mod(nSamples,p.data.mapEvalMod) % Save acquisition models for analysis later
        output.models{nSamples,iModel} = model{iModel};
        %end
    end
    
    % Save found model parameters and new acquisition function
    for iModel=1:size(value,2)
        if strcmp(d.params{iModel}.name,'GP'); d.params{iModel}.hyp = model{iModel}.hyp; end
    end
    if isfield(p, 'constraints')
        d.constraints = p.constraints; % Send constraints to domain
    end
    ...
    pPred.nChildren         = 128;
    pPred.nGens             = 1024;
    pPred.display.illu      = false;
    [predMap(nSamples), predPercImprove(:,nSamples)] = createPredictionMap(model,pPred,d,'featureRes',[25 25]);
    if p.display.illu
        figure(4); clf;
        [h(1), h(2)] = viewMap(-predMap(nSamples).fitness, d, predMap(nSamples).edges); title('Predicted Fitness')
        caxis([0 6]);
        colormap(h(1),parula(16));
        drawnow;
        figure(5); clf;
        [h(3), h(4)] = viewMap(predMap(nSamples).confidence, d, predMap(nSamples).edges); title('Prediction Model Variance')
        caxis([0 1]);
        colormap(h(3),parula(32));
        drawnow;
        
        diversityMap = getDiversityMap(predMap(nSamples).genes);
        figure(6); viewMap(diversityMap,d);title('Genetic Diversity');
        colormap(parula(16));
        caxis([0 0.5]);
        diversityMapRecord{nSamples} = diversityMap;
    end
...
if exist('predMap');output.predMapRecord = predMap;end
output.diversityMap = diversityMapRecord;
