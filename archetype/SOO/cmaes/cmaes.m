...
%CMAES Version 3.61.beta, Hansen
...
% These defaults may be edited for convenienc'Base polygon', e
...
while irun <= myeval(opts.Restarts) % for-loop does not work with resume
    ...
    while isempty(stopflag)
        ...
        if flgEvalParallel
            ...
            %% PARALLEL FITNESS EVALUATION HERE
%             for iInd = 1:size(arxvalid,2) 
%                 for iArg = 1:length(varargin)
%                    arguments{iInd,iArg} = varargin{iArg}; 
%                 end
%                 arguments{iInd,length(varargin)} = iInd; 
%             end
%             
%             parfor iInd = 1:size(arxvalid,2)        
%                 fit(iInd) = feval(fitfun, arxvalid(:,iInd), arguments{iInd,:});
%             end

            parfor iInd = 1:size(arxvalid,2)
                fit(iInd) = feval(fitfun, arxvalid(:,iInd), varargin{:});
            end
            fitness.raw = fit;
        ...
        for k=find(isnan(fitness.raw)),
            ...
            % Resample, until fitness is not NaN or tried too many times
            while isnan(fitness.raw(k)) && isempty(stopflag)
                ...
                if mod(tries, 25) == 0
                    % If we can't find a valid value after this many tries,
                    % take the best individual found so far
                    
                    arxvalid(:,k) = bestever.x;
                    fitness.raw(k) = feval(fitfun, arxvalid(:,k), varargin{:});
                    
                    %                     if stopOnWarnings
                    %                         stopflag(end+1) = {'constraintViolation'};
                    warning([num2str(tries) ...
                        ' NaN objective function values at evaluation ' ...
                        num2str(counteval) ' setting individual to previous best']);
                    %                         continue;
                    %                     end
                end
                if mod(tries, 99) == 0
        ...
        out.evals = counteval;
        out.history = fitness.hist(end:-1:1);
        ...
        %% ----- output generation -----
        fitHistory(countiter,:) = [counteval, fitness.raw(1), median(fitness.raw)];
    ...
    out.fitHistory = fitHistory;
    %% -------------------- Final Procedures -------------------------------
    
    % Evaluate xmean and return best recent point in xmin
    fmin = fitness.raw(1);
    if ~isfield(fitness,idx),fitness.idx(1) = 1;end;
