function [map, percImproved, percValid, allMaps, percFilled, stats] = illuminate(map,fitnessFunction,p,d,varargin)
...
while (iGen <= p.nGens)
    ...
    [fitness] = fitnessFunction(children); %% TODO: Speed up without anonymous functions
    
    %% Add Children to Map
    phenotypes = d.getPhenotype(children);
    features = d.categorize(children,phenotypes,p,d);
...
%         plot(100*map.stats.fitnessMean);
%         hold on;
...
%         grid on;
%         %axis([1 length(map.stats.fitnessTotal) 0 p.maxBins]);
%         drawnow;
    end
    iGen = iGen+1;
