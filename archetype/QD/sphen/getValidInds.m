function [inds, vals, feats, nMissing, nAttempts] = getValidInds(indPool,testFunction,nDesired,retryInvalid)
...
while nMissing > 0
    ...
    % Test for validity
    [result,features] = testFunction(nextInd); % Must return a [nInds X nVals] matrix
    
    % Assign valid solutions
    validInds = find( any(~isnan(result),2) );
    vals = [vals;  result(validInds,:)]; %#ok<AGROW>
    feats = [feats;  features(validInds,:)]; %#ok<AGROW>
