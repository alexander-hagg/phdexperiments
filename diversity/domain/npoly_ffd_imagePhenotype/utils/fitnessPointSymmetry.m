function [fitness,polygons,rawFitness] = fitnessPointSymmetry(genomes,d)
...
for i=1:length(polygons)
    ...
    if isa(polygons{i},'double') || polygons{i}.NumRegions == 0
        ...
        a = pt(1:end/2,:); % Take first half of points
        x = pt(end/2+1:end,:); % Second half of points
        rawFitness(i) = sum(sqrt(sum(((a+x)'.^2),1)));
